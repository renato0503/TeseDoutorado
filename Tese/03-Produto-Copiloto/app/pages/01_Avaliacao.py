...
if analisar and texto.strip():
    ...
    with st.spinner("Executando TF-IDF + Isolation Forest + Random Forest + SHAP..."):
        resultado = analisar_risco_contratual(texto)
        anomalia = detectar_anomalia(texto)
        recomendacoes = gerar_recomendacoes(resultado["lacunas"], resultado["clausulas_encontradas"])
    ...
    <div class="result-card">
        ...
        <p> TF-IDF + Isolation Forest (treinado em 15.000 objetos PNCP)<br>
         ...
         SHAP TreeExplainer (7 features)</p>
        <p><strong>Anomalia:</strong> {anomalia['mensagem']}</p>
        <p style="margin-top:12px;font-size:0.75rem;color:rgba(255,255,255,0.6);">
            Treinado em {datetime.now().strftime('%Y-%m-%d')} sobre 50.000 contratos do PNCP (Set/2021 - Ago/2024).
        </p>
    </div>
    """, unsafe_allow_html=True)
