...
with col_m2:
    ...
    st.markdown(f'<div class="model-status {status_class}">Random Forest: <strong>{status_text}</strong><br><small>50.000 contratos, 7 features, AUC={metricas.get("auc_roc", 0)*100:.2f}%</small></div>', unsafe_allow_html=True)
with col_m3:
...
with col_x2:
    st.markdown('<div class="feature-card"><h3>2. Random Forest</h3><p>Prediz o risco com base em 7 features (valor, complexidade lexica, score tecnico, UF, tipo) treinadas em 50k contratos.</p></div>', unsafe_allow_html=True)
with col_x3:
